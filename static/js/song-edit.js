...
(function() {
  ...
  // ===== Similarity Groups Management =====
  let similarityGroups = [];
  let simGroupSelecting = false;
  let simGroupSelectedIds = [];
  let simGroupType = 'identical';

  // Load similarity groups from song data
  function loadSimilarityGroups() {
    const root = document.getElementById('exercise-view');
    if (!root || !root.dataset.song) return;
    try {
      const song = JSON.parse(root.dataset.song);
      similarityGroups = song.similarityGroups || [];
    } catch(e) {
      similarityGroups = [];
    }
  }
  ...
  // Show similarity group panel in stats drawer
  window.openSimilarityManager = function() {
    var panel = document.getElementById('sim-group-panel');
    if (!panel) return;
    panel.style.display = 'block';
    renderSimilarityGroups();
  };

  window.closeSimilarityManager = function() {
    var panel = document.getElementById('sim-group-panel');
    if (panel) panel.style.display = 'none';
    simGroupSelecting = false;
    simGroupSelectedIds = [];
    clearSimGroupHighlights();
  };

  window.startSimGroupSelect = function(type) {
    simGroupType = type;
    simGroupSelecting = true;
    simGroupSelectedIds = [];
    renderSimGroupSelectUI();
  };

  window.cancelSimGroupSelect = function() {
    simGroupSelecting = false;
    simGroupSelectedIds = [];
    clearSimGroupHighlights();
    renderSimilarityGroups();
  ...
  window.toggleSimGroupCard = function(exerciseId) {
    var idx = simGroupSelectedIds.indexOf(exerciseId);
    if (idx >= 0) {
      simGroupSelectedIds.splice(idx, 1);
    } else {
      simGroupSelectedIds.push(exerciseId);
    }
    renderSimGroupSelectUI();
  };

  window.confirmSimGroup = function() {
    if (simGroupSelectedIds.length < 2) return;

    var group = {
      id: generateSimGroupId(),
      type: simGroupType,
      exerciseIds: simGroupSelectedIds.slice()
    };
    similarityGroups.push(group);
    saveSimilarityGroups();
    simGroupSelecting = false;
    simGroupSelectedIds = [];
    clearSimGroupHighlights();
    renderSimilarityGroups();
  ...
  window.deleteSimGroup = function(groupId) {
    similarityGroups = similarityGroups.filter(function(g) { return g.id !== groupId; });
    saveSimilarityGroups();
    renderSimilarityGroups();
  };

  function saveSimilarityGroups() {
    fetch('/api/songs/' + songId + '/similarity-groups', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(similarityGroups)
    }).catch(console.error);
  }

  function generateSimGroupId() {
    return 'sg_' + Math.random().toString(36).substring(2, 10);
  }

  function renderSimilarityGroups() {
    ...
    similarityGroups.forEach(function(group) {
      var div = document.createElement('div');
      ...
      var exerciseNames = group.exerciseIds.map(function(id) {
        var ex = existingExercises.find(function(e) { return e.id === id; }) ||
                 exercises.find(function(e) { return e.id === id; });
        return ex ? (ex.name || ex.description || 'Exercise') : 'Unknown';
      });

      div.innerHTML = typeBadge +
        '<span class="sim-group-names">' + exerciseNames.join(' <span class="sim-group-link-icon">↔</span> ') + '</span>' +
  ...
  function renderSimGroupSelectUI() {
    var container = document.getElementById('sim-group-list');
    if (!container) return;

    var typeLabel = simGroupType === 'identical' ? 'Identical' : 'Similar';
    var html = '<div class="sim-group-select-header">' +
      '<span>Select exercises for <strong>' + typeLabel + '</strong> group</span>' +
      '<button class="sim-group-cancel-btn" onclick="cancelSimGroupSelect()">Cancel</button>' +
    '</div>';

    html += '<div class="sim-group-card-list">';
    var allExercises = existingExercises.length > 0 ? existingExercises : exercises;
    allExercises.forEach(function(ex) {
      var name = ex.name || ex.description || 'Exercise';
      var selected = simGroupSelectedIds.indexOf(ex.id) >= 0;
      html += '<button class="sim-group-card-btn' + (selected ? ' selected' : '') + '" onclick="toggleSimGroupCard(\'' + ex.id + '\')">' +
        '<span class="sim-group-card-check">' + (selected ? '✓' : '') + '</span>' +
        '<span>' + escHtml(name) + '</span>' +
      '</button>';
    });
    html += '</div>';

    if (simGroupSelectedIds.length >= 2) {
      html += '<button class="sim-group-confirm-btn" onclick="confirmSimGroup()">Create Group (' + simGroupSelectedIds.length + ' exercises)</button>';
    } else {
      html += '<div class="sim-group-hint">Select at least 2 exercises</div>';
    }

    container.innerHTML = html;
  }

  function clearSimGroupHighlights() {
    document.querySelectorAll('.expanded-card-wrapper.sim-highlight').forEach(function(el) {
